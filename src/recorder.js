...
    p.className = event.results[event.results.length - 1].isFinal ? "final" : "interim";
...
    console.log("Usuario cancela interacción con IA por completo.");
    
    // Forzar la cancelación de cualquier síntesis de voz en curso o en cola
    synth.cancel(); 
    
...
    // Invocar IA con streaming y pasar callbacks para UI
    sendMensajeIANormal(
      userText,
      (chunk) => {
        if (isIAThinking) {
...
        checkAndShowNewChatButton(); // Llama a la función para verificar el botón
        // Aquí no llamamos a resetAfterIA() directamente.
        // El evento 'iaspeech:ended' (desde speak.ts) se encargará de resetear
        // cuando la voz termine su reproducción natural.
      },
      () => {
...
        if (!newChatButton) { // Si el botón no existe, lo creamos
...
            // Event Listener para recargar la página
